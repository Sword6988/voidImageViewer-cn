# -*- coding: utf-8 -*-
"""带图片启动，抓主窗口（工具栏/状态栏/标题）+ Jump To 列表"""
import ctypes, ctypes.wintypes as wt, subprocess, time, zlib, struct, sys, os

u32 = ctypes.windll.user32
gdi32 = ctypes.windll.gdi32
INST = r"C:\Program Files\voidImageViewer"
EXE = os.path.join(INST, "voidImageViewer.exe")
OUT = sys.argv[1]
PREFIX = sys.argv[2]
IMGS = [r"D:\Desktop\软件汉化\test\test1.png", r"D:\Desktop\软件汉化\test\test2.png", r"D:\Desktop\软件汉化\test\test3.png"]
os.makedirs(OUT, exist_ok=True)

for f, a, r in [('GetWindowTextW', [wt.HWND, wt.LPWSTR, ctypes.c_int], None),
                ('GetClassNameW', [wt.HWND, wt.LPWSTR, ctypes.c_int], None),
                ('GetMenuStringW', [wt.HMENU, ctypes.c_uint, wt.LPWSTR, ctypes.c_int, ctypes.c_uint], None),
                ('GetMenu', [wt.HWND], ctypes.c_void_p),
                ('GetSubMenu', [ctypes.c_void_p, ctypes.c_int], ctypes.c_void_p),
                ('GetMenuItemCount', [ctypes.c_void_p], ctypes.c_int),
                ('SendMessageW', [wt.HWND, ctypes.c_uint, ctypes.c_ulonglong, ctypes.c_void_p], ctypes.c_longlong),
                ('PrintWindow', [wt.HWND, wt.HDC, ctypes.c_uint], None)]:
    fn = getattr(u32, f)
    fn.argtypes = a
    if r:
        fn.restype = r


def text(h):
    b = ctypes.create_unicode_buffer(2048)
    u32.GetWindowTextW(h, b, 2048)
    return b.value


def cls(h):
    b = ctypes.create_unicode_buffer(256)
    u32.GetClassNameW(h, b, 256)
    return b.value


def pid_of(h):
    p = wt.DWORD()
    u32.GetWindowThreadProcessId(h, ctypes.byref(p))
    return p.value


def enum_top(pid):
    res = []
    CB = ctypes.WINFUNCTYPE(ctypes.c_bool, wt.HWND, wt.LPARAM)

    def cb(h, l):
        if pid_of(h) == pid and u32.IsWindowVisible(h):
            res.append(h)
        return True
    u32.EnumWindows(CB(cb), 0)
    return res


def kids(hwnd):
    res = []
    CB = ctypes.WINFUNCTYPE(ctypes.c_bool, wt.HWND, wt.LPARAM)

    def cb(h, l):
        res.append((cls(h), text(h), h))
        return True
    u32.EnumChildWindows(hwnd, CB(cb), 0)
    return res


def write_png(path, w, h, bgra):
    rows = []
    for y in range(h):
        line = bytearray()
        for x in range(w):
            i = (y * w + x) * 4
            line += bytes((bgra[i + 2], bgra[i + 1], bgra[i]))
        rows.append(b'\x00' + bytes(line))
    raw = b''.join(rows)

    def chunk(t, d):
        return struct.pack('>I', len(d)) + t + d + struct.pack('>I', zlib.crc32(t + d) & 0xffffffff)
    png = b'\x89PNG\r\n\x1a\n' + chunk(b'IHDR', struct.pack('>IIBBBBB', w, h, 8, 2, 0, 0, 0)) \
        + chunk(b'IDAT', zlib.compress(raw, 6)) + chunk(b'IEND', b'')
    open(path, 'wb').write(png)


def capture(hwnd, path):
    r = wt.RECT()
    u32.GetWindowRect(hwnd, ctypes.byref(r))
    w, h = r.right - r.left, r.bottom - r.top
    if w <= 0 or h <= 0:
        return (0, 0)
    hdc = u32.GetWindowDC(hwnd)
    mdc = gdi32.CreateCompatibleDC(hdc)
    bmp = gdi32.CreateCompatibleBitmap(hdc, w, h)
    gdi32.SelectObject(mdc, bmp)
    u32.PrintWindow(hwnd, mdc, 2)
    bi = struct.pack('<IiiHHIIiiII', 40, w, -h, 1, 32, 0, 0, 0, 0, 0, 0)
    buf = ctypes.create_string_buffer(w * h * 4)
    gdi32.GetDIBits(mdc, bmp, 0, h, buf, ctypes.byref(ctypes.create_string_buffer(bi)), 0)
    write_png(path, w, h, buf.raw)
    return (w, h)


IDMAP = {}


def dump_menu(hm, depth=0, out=None):
    if out is None:
        out = []
    for i in range(u32.GetMenuItemCount(hm)):
        b = ctypes.create_unicode_buffer(512)
        u32.GetMenuStringW(hm, i, b, 512, 0x400)
        sub = u32.GetSubMenu(hm, i)
        mid = u32.GetMenuItemID(hm, i)
        label = b.value.split('\t')[0]
        if not sub:
            out.append(('  ' * depth) + b.value + '  {id=%d}' % mid)
            IDMAP.setdefault(label, mid)
        else:
            out.append(('  ' * depth) + b.value)
            dump_menu(sub, depth + 1, out)
    return out


proc = subprocess.Popen([EXE] + IMGS, cwd=INST)
print('PID', proc.pid)
try:
    main = None
    for _ in range(60):
        time.sleep(0.25)
        for h in enum_top(proc.pid):
            if u32.GetMenu(h):
                main = h
                break
        if main:
            break
    print('标题:', text(main))
    # 子控件（工具栏/状态栏）
    print('--- 子控件 ---')
    for c, t, h in kids(main):
        print('   [%s] %r' % (c, t))
    lines = dump_menu(u32.GetMenu(main))
    open(os.path.join(OUT, PREFIX + '_menu.txt'), 'w', encoding='utf-8').write('\n'.join(lines))
    time.sleep(0.8)
    print('截图:', capture(main, os.path.join(OUT, PREFIX + '_main_img.png')))

    # 状态栏文字（读 status bar 的 parts）
    # Jump To
    u32.PostMessageW(main, 0x0111, IDMAP.get('&Jump To...', 0), 0)
    dlg = None
    for _ in range(40):
        time.sleep(0.2)
        for h in enum_top(proc.pid):
            if cls(h) == '#32770' and h != main:
                dlg = h
                break
        if dlg:
            break
    if dlg:
        print('\n=== Jump To 内容 ===')
        for c, t, h in kids(dlg):
            print('   [%s] %r' % (c, t))
            if 'ListBox' in c and 'ListView' not in c:
                n = u32.SendMessageW(h, 0x018B, 0, 0)
                for i in range(min(n, 40)):
                    ln = u32.SendMessageW(h, 0x018A, i, 0)
                    if ln <= 0:
                        continue
                    buf = ctypes.create_unicode_buffer(ln + 2)
                    u32.SendMessageW(h, 0x0189, i, ctypes.cast(buf, ctypes.c_void_p))
                    print('        - %r' % buf.value)
        time.sleep(0.4)
        capture(dlg, os.path.join(OUT, PREFIX + '_jumpto_img.png'))
        u32.PostMessageW(dlg, 0x0010, 0, 0)
    else:
        print('!! 无 Jump To 对话框')

    # 全屏 & 状态栏像素信息
    time.sleep(0.6)
    u32.PostMessageW(main, 0x0010, 0, 0)
    for _ in range(20):
        if proc.poll() is not None:
            break
        time.sleep(0.25)
finally:
    if proc.poll() is None:
        proc.terminate()
        try:
            proc.wait(timeout=5)
        except Exception:
            proc.kill()
    print('exit', proc.returncode)
